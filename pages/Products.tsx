...
import { Search, Filter, ShoppingCart, Info, Check, Tag } from 'lucide-react';
    ...
    <div className="bg-gray-50 min-h-screen py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div>
            <h1 className="text-4xl font-display font-bold text-primary mb-2">Institutional Catalog</h1>
            <p className="text-gray-600">Premium solutions with full GST & HSN compliance.</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
            ...
            >
              <option value="all">All Enterprise Categories</option>
              {CATEGORIES.map(c => (
          ...
          <div className="hidden md:block space-y-6">
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
              <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
                <Filter size={18} /> Navigation
              </h3>
              <div className="space-y-2">
                <button 
                  onClick={() => setSelectedCategory('all')}
                  className={`block w-full text-left px-3 py-2 rounded-lg transition-colors font-bold ${selectedCategory === 'all' ? 'bg-primary text-white' : 'text-gray-600 hover:bg-gray-50'}`}
                >
                  All Items
                </button>
                {CATEGORIES.map(c => (
                  ...
                  >
                    ...
                    <div className={`text-[10px] uppercase ${selectedCategory === c.id ? 'text-blue-100' : 'text-gray-400'}`}>Institutional Solution</div>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="md:col-span-3">
            {filteredProducts.length > 0 ? (
                ...
                {filteredProducts.map(p => (
                  <div key={p.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden group hover:shadow-xl transition-all flex flex-col h-full">
                    <div className="aspect-[4/3] relative overflow-hidden bg-gray-100">
                      ...
                      {p.isFeatured && (
                        <div className="absolute top-4 right-4 bg-secondary text-white px-2 py-1 rounded-md text-[10px] font-bold shadow-sm uppercase tracking-widest">
                          Premium
                        </div>
                      )}
                    </div>
                    <div className="p-5 flex-grow flex flex-col">
                      ...
                      {p.hsnCode && (
                        <div className="flex items-center gap-2 text-[10px] font-bold text-gray-400 mb-4 bg-gray-50 w-fit px-2 py-1 rounded">
                          <Tag size={10} /> HSN: {p.hsnCode} | GST: {p.gstRate}%
                        </div>
                      )}
              ...
              <div className="bg-white rounded-2xl p-24 text-center border-2 border-dashed border-gray-200">
                ...
                <p className="text-gray-500">Contact our technical expert for custom sourcing.</p>
              </div>
            )}
          </div>
        </div>
